...
static void	_handle_first_endpoint(t_line_conf *lc, t_point p0, t_ctx *ctx)
{
	double	x_end;
	double	y_end;
	double	x_gap;
	...
	if (lc->steep)
	{
		plot_point_to_image(ctx, lc->y_pxl1, lc->x_pxl1,
			color_gradient(0x0, lc->color_start, rfpart(y_end * x_gap)));
		plot_point_to_image(ctx, lc->y_pxl1 + 1, lc->x_pxl1,
			color_gradient(0x0, lc->color_start, fpart(y_end * x_gap)));
	}
	else
	{
		plot_point_to_image(ctx, lc->x_pxl1, lc->y_pxl1,
			color_gradient(0x0, lc->color_start, rfpart(y_end * x_gap)));
		plot_point_to_image(ctx, lc->x_pxl1, lc->y_pxl1 + 1,
			color_gradient(0x0, lc->color_start, fpart(y_end * x_gap)));
	}
	lc->intery = y_end + lc->gradient;
}

static void	_handle_second_endpoint(t_line_conf *lc, t_point p1, t_ctx *ctx)
{
	double	x_end;
	double	y_end;
	double	x_gap;
	...
	if (lc->steep)
	{
		plot_point_to_image(ctx, lc->y_pxl2, lc->x_pxl2,
			color_gradient(0x0, lc->color_end, rfpart(y_end * x_gap)));
		plot_point_to_image(ctx, lc->y_pxl2 + 1, lc->x_pxl2,
			color_gradient(0x0, lc->color_end, fpart(y_end * x_gap)));
	}
	else
	{
		plot_point_to_image(ctx, lc->x_pxl2, lc->y_pxl2,
			color_gradient(0x0, lc->color_end, rfpart(y_end * x_gap)));
		plot_point_to_image(ctx, lc->x_pxl2, lc->y_pxl2 + 1,
			color_gradient(0x0, lc->color_end, fpart(y_end * x_gap)));
	}
}

void	_main_loop(t_line_conf lc, t_point p0, t_point p1, t_ctx *ctx)
{
	int		x;
	int		color;
	double	ratio;
	...
	while (++x < lc.x_pxl2 - 1)
	{
		...
		if (lc.steep)
		{
			plot_point_to_image(ctx, floor(lc.intery), x,
				color_gradient(0x0, color, rfpart(lc.intery)));
			plot_point_to_image(ctx, floor(lc.intery) + 1, x,
				color_gradient(0x0, color, fpart(lc.intery)));
		}
		else
		{
			plot_point_to_image(ctx, x, floor(lc.intery),
				color_gradient(0x0, color, rfpart(lc.intery)));
			plot_point_to_image(ctx, x, floor(lc.intery) + 1,
				color_gradient(0x0, color, fpart(lc.intery)));
		}
...
void	plot_line(t_point p0, t_point p1, t_ctx *ctx)
{
	...
	_handle_first_endpoint(&lc, p0, ctx);
	_handle_second_endpoint(&lc, p1, ctx);
	_main_loop(lc, p0, p1, ctx);
}
